document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateBalanceDisplay() {
        ...
        balanceDisplay.innerText = `Your current balance is: Rp ${balanceInt.toLocaleString(
            "id-ID"
        )}`;
    }
    ...
    function createMovieHTML(movieDetails) {
      ...
      <div class="movie-details">
        ...
        <div class="movie-info">
          ...
          <p class="movie-release-date">Release Date: ${
              movieDetails.release_date
          }</p>
          <p class="movie-ticket-price">Ticket Price: Rp ${movieDetails.ticket_price.toLocaleString(
              "id-ID"
          )}</p>
          <button class="selectSeat" data-movie-info='${JSON.stringify(
              movieDetails
          )}'>Select Seat</button>
        </div>
      </div>
